...
extern int petimer_start_abstimer(HI_VOID *sync_block, petimer_t timer, struct tm * tm, HI_VOID * arg) {
    ...
    info->tm = *tm;
    info->arg = arg;
    ret = tsklet_invokelater(&sync_blk->abs_tsklets, (HI_VOID (*)(HI_VOID*, HI_VOID*))_starttimer, info);
    if(ret) {
        free(info);
    }
    return ret;
...
extern int petimer_stop_abstimer_safe(HI_VOID *sync_block, petimer_t timer, HI_VOID (*finallize)(HI_VOID *), HI_VOID * arg) {
    ...
    info->arg = arg;
    ret = tsklet_invokelater(&sync_blk->abs_tsklets, (HI_VOID (*)(HI_VOID*, HI_VOID*))_stoptimer, info);
    if(ret) {
        free(info);
    }
    return ret;
...
extern int petimer_free_abstimer(HI_VOID *sync_block, petimer_t timer) {
    sync_block_t *sync_blk = (sync_block_t *)sync_block;
    return tsklet_invokelater(&sync_blk->abs_tsklets, (HI_VOID (*)(HI_VOID*, HI_VOID*))_rmtimer, timer);
}
