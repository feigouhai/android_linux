...
#output prefix set
obj_prefix :=$(OBJ_DIR)/$(LOCAL_MODULE).EXE
bin_prefix :=$(BIN_DIR)
...
target_cc_source := $(filter %.cc, $(LOCAL_SRC_FILES))


target_cc_objs   := $(addprefix $(obj_prefix)/,$(target_cc_source:.cc=.o))


ifneq ($(strip $(target_cc_objs)),)
$(target_cc_objs):$(obj_prefix)/%.o:$(TOP_DIR)$(LOCAL_PATH)/%.cc
...
target_all_objs := $(target_c_objs) $(target_cpp_objs) \
                   $(target_cc_objs) $(target_s_objs)

target_all_source := $(target_c_source) $(target_cpp_source) \
                     $(target_cc_source) $(target_s_source)
...
$(bin_prefix)/$(LOCAL_MODULE)$(LOCAL_MODULE_SUFFIX):$(all_required_modules) $(target_all_objs) $(all_libraries)
	@echo "<<< compile bin $@:"
	@mkdir -p $(dir $@)
	$(Q)$(PRIVATE_CXX) \
...
$(LOCAL_MODULE)-t-bin-clean:
	...
	$(Q)rm -rf $(PRIVATE_BIN_DIR)/$(PRIVATE_LOCAL_MODULE)$(PRIVATE_MODULE_SUFFIX)
