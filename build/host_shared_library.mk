...
LOCAL_MODULE_SUFFIX := $(HOST_SHAREDLIB_SUFFIX)
endif
...
#output prefix set
obj_prefix :=$(HOST_OBJ_DIR)/$(LOCAL_MODULE).SHARED
...
###########################################################
##
## C++: compile .cc source files to .o
##
###########################################################

host_cc_source := $(filter %.cc, $(LOCAL_SRC_FILES))


host_cc_objs   := $(addprefix $(obj_prefix)/,$(host_cc_source:.cc=.o))


ifneq ($(strip $(host_cc_objs)),)
$(host_cc_objs):$(obj_prefix)/%.o:$(TOP_DIR)$(LOCAL_PATH)/%.cc
	@echo "<<< compile cc objs $@:"
	@mkdir -p $(dir $@)
	$(Q)$(PRIVATE_CXX) \
        $(addprefix -I , $(PRIVATE_C_INCLUDES)) \
        -c $(PRIVATE_CPPFLAGS)  -o $@ $^
...
host_all_objs := $(host_c_objs) $(host_cpp_objs) \
                 $(host_cc_objs) $(host_s_objs)

host_all_source := $(host_c_source) $(host_cpp_source) \
                   $(host_cc_source) $(host_s_source)
...
LOCAL_INTERMEDIATE_TARGETS := $(host_all_objs) $(host_all_source) \
                              ...
                              $(LOCAL_MODULE)-h-shlib-check \
                              $(LOCAL_MODULE)-h-shlib-clean
...
$(LOCAL_MODULE)-check:$(LOCAL_MODULE)-h-shlib-check
$(LOCAL_MODULE)-clean:$(LOCAL_MODULE)-h-shlib-clean

$(lib_prefix)/$(LOCAL_MODULE)$(LOCAL_MODULE_SUFFIX):$(all_required_modules) $(host_all_objs) $(all_libraries)
